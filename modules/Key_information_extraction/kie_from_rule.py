...
def get_kie(image, bboxes, labels, ios=0.5, max_dis_y=3):
    ...
    for idx in range(len(classes)):
        if check_keys(labels[idx].lower(), ignore_pattern):
            classes[idx] = -1
    ...
    for key_idx in keys_pattern:
        ...
        if idx != -1:
                    ...
                    if check_intersection_over_searchbbox(bboxes[idx_value], search_area) > ios \
                        ...
                        if match_value_idx == -1:
                            if key_idx == 22 or key_idx == 23:
                                # if bboxes[idx_value][0] < bboxes[idx][2]:
            ...
            if match_value_idx != -1:
                ...
                dict_file["{0:02d}_".format(key_idx) + mapping[key_idx]] = labels[match_value_idx]
            else:
                # dict_file[mapping[key_idx]] = ''
                dict_file["{0:02d}_".format(key_idx) + mapping[key_idx]] = ''
